...
impl State {
    pub fn new(rom: Box<Mapper>, output_texture: Arc<Mutex<Bitmap>>) -> Self {
        ...
        // Initialise bus to the value that the PPU data register would return
        // on the first read of $2002.  The reference logs expect this to be
        // 0x10, so we set it explicitly here.
        let bus = 0x10;
    ...
    fn read_ppu(&mut self, adr: u16) -> u8 {
        ...
        // Update the bus with the value that was just read so that subsequent
        // reads of $2000/$2001/$2003/$2004/$2005/$2006/$2007 return the
        // correct value.
        self.bus = res;
        res
    }
